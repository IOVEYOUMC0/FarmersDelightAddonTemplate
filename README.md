...
| `apiVersion()` / `hasFeature(String)` | version + feature gating (see [Versioning](#versioning--feature-gating)) |
...
`isAvailable()` answers "is FarmersDelight running". `apiVersion()` / `hasFeature(String)` answer "does this
FD build expose the surface I compiled against" — gate on these, not on the plugin's version string (the api
version tracks the addon-facing surface; the plugin version tracks content). `apiVersion()` is a monotonic
revision, bumped by +1 on every additive release and never decremented; nothing is ever removed, so an addon
compiled against revision N keeps linking against every later revision. `hasFeature(id)` is the direct probe
for a single capability (`"station-query"`, `"harvest-event"`, `"cook-start-event"`, `"buff-change-event"`,
`"cooking-experience-location"`, `"knife-drop-rules"`, `"compat-util"`, `"recipes"`, `"item-displays"`,
`"scheduler"`, `"buffs"`, `"debug-tools"`); an unknown/null id returns `false`.

On a build older than the one that introduced these methods the call throws `NoSuchMethodError` — catch it and
treat it as revision 0. `FDAddonTemplate.onEnable()` shows the pattern:

```java
int apiVersion;
try {
    apiVersion = FarmersDelightApi.get().apiVersion();
} catch (NoSuchMethodError pre) {
    apiVersion = 0; // pre-versioning FD build
}
...
- `api.recipe.*` — `RecipeType`, `ViewableRecipe`, `RecipeFiller`, `RecipeEditor`, `EditableRecipe`,
  ...
  - **Fill feedback**: a `RecipeFiller` may override `fillDetailed(player, recipe, fillAll)` to return a
    `FillOutcome` (`FILLED` / `MISSING_INGREDIENTS` / `INVENTORY_FULL` / `NOTHING`) so the book mirrors the
    cooking pot's fill-button states; `fillAll` distinguishes a shift-click (whole recipe) from a single-click
    (one of each). The default adapts the boolean `fill(...)`, so a legacy filler still works — only without
    the richer states. Only `FILLED` should reopen the station GUI; the others stay on the book so the
...
- `api.event.*` — `FarmersDelightReloadEvent`, `FarmersDelightProduceEvent`, `ProfessionCookingExperienceEvent`,
  the gameplay notifications `FarmersDelightCookStartEvent` (idle→cooking edge, counterpart to ProduceEvent),
  `FarmersDelightHarvestEvent` (mushroom-colony / rice Java harvests), `FarmersDelightBuffChangeEvent` (real
  custom-buff level transitions, not refreshes) and `FarmersDelightRecipeDiscoveryEvent` (per-player recipe
  lock/unlock transitions), plus the admin / lifecycle bridges `FarmersDelightCleanupEvent`,
  `FarmersDelightMigrateEvent`, `FarmersDelightWarmupEvent` and `FarmersDelightCollectLiveDisplaysEvent` — see
  `listener/ExampleFarmersDelightEventsListener` (and the display manager for collect-live-displays). None are
  cancellable: they are notifications, and vetoing belongs upstream (harvest vetoes belong to the protection
  layer FarmersDelight already consults).
- `api.util.*` — `PluginManagerGuard` (blocks a live PlugMan reload/unload of your plugin; register it in
  onEnable), `TooltipUtils.hideDurabilityLine` (hide the durability line when an item's damage encodes a
...
## More API surface (text, items, effects, config updates, advancements, recipe queries, discovery, blocks, knife drops)
...
The template ships this as [`util/ExampleConfigBootstrap`](src/main/java/com/example/fdaddon/util/ExampleConfigBootstrap.java),
modelled on BrewinAndChewin's production `BrewinConfigBootstrap`; `FDAddonTemplate.onEnable()` calls
`configBootstrap.updateConfig()` right after `saveDefaultConfig()` and before reading any value. The shape:
...
### Block station-query — `api.block.FarmersDelightBlocks` (+ `FarmersDelightStation`, `*Snapshot`)

Read-only identification of FarmersDelight's four workstations and what each currently holds. Every parameter
and return type is a Bukkit / java type (plus the snapshot records), so no CraftEngine dependency is needed.
See `readCookingPot` in `FDAddonTemplate.java`.

```java
if (FarmersDelightBlocks.isStation(block)) {
    FarmersDelightStation which = FarmersDelightBlocks.stationOf(block);  // COOKING_POT / CUTTING_BOARD / SKILLET / STOVE
    String blockId = FarmersDelightBlocks.blockIdOf(block);              // any CE custom block id, or null
    CookingPotSnapshot pot = FarmersDelightBlocks.cookingPot(block);     // null unless it is a cooking pot
    if (pot != null && pot.cooking()) {
        double p = pot.progressFraction();                               // 0..1
    }
}
// also: cuttingBoard(block) -> CuttingBoardSnapshot, skillet(block) -> SkilletSnapshot, stove(block) -> StoveSnapshot,
// stationIdOf(block).
```

Two hard rules from the API's own javadoc:

- **Never identify a station by `Material`.** CraftEngine reports a configurable disguise material (the same
  one for every custom block by default), so `getType() == Material.X` identifies nothing and changes silently
  when the server owner edits CraftEngine's disguise setting. Always go through `isStation` / `stationOf` /
  `stationIdOf`, which resolve the real custom block state (detection is by the attached CraftEngine behavior,
  so a re-skinned or addon-provided station is still recognised). `FarmersDelightStation.defaultBlockId()` is
  the stock id FD ships each station under — not what `stationIdOf` returns for a re-skinned block.
- **Call on the region thread that owns the block** (Folia / Luminol): every method reads world state, so call
  from a listener for an event at that block, or a task scheduled with `runAtLocation`. Calling from another
  region, the global region, or an async task hits Paper's thread check and throws. A snapshot is a copy taken
  at the instant of the call (the records are immutable and write nothing back) — treat it as a reading, not a
  live handle. `null` means "not that station, or no block entity loaded".

### Knife drops — `api.loot.FarmersDelightKnifeDrops`

Register extra mob drops harvested with a knife at runtime (the rule set behind FD's ham / leather / feather /
string drops), for an addon that ships its own butchering items or a quest plugin adding a conditional drop.
A rule fires when a player kills an adult entity of the matching type with a matching tool and a chance roll
passes; there is one rule per entity type. Rules registered here persist across `/fd reload` (the reload
re-applies them on top of `config.yml`), so unregister on your plugin's disable.

```java
FarmersDelightKnifeDrops.register("pig", "myaddon:raw_ham", "myaddon:cooked_ham", 0.35, 0.05);
// full form adds per-rule tool matchers (item ids + "#tag" ids); empty/null falls back to the global knife list:
FarmersDelightKnifeDrops.register("cow", "myaddon:tallow", null, 0.20, 0.0,
        List.of("myaddon:cleaver"), List.of("minecraft:swords"));
FarmersDelightKnifeDrops.unregister("pig");                 // only removes rules this facade added
boolean has = FarmersDelightKnifeDrops.hasRule("cow");      // from any source (config / built-in / facade)
```
