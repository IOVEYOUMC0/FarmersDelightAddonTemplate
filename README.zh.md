...
| `apiVersion()` / `hasFeature(String)` | 版本 + 功能门控（见下文“版本与功能门控”一节） |
...
`isAvailable()` 回答“FarmersDelight 是否在运行”。`apiVersion()` / `hasFeature(String)` 回答“当前 FD 构建是否
暴露了我编译时所依赖的那部分接口” —— 请以这两者门控，而不是以插件的版本号（api 版本跟踪面向附属的接口，
插件版本跟踪内容）。`apiVersion()` 是单调递增的修订号，每次增量发布 +1、从不回退；接口从不删除，因此针对
修订号 N 编译的附属能持续链接到之后的每个修订。`hasFeature(id)` 是对单个能力的直接探测（`"station-query"`、
`"harvest-event"`、`"cook-start-event"`、`"buff-change-event"`、`"cooking-experience-location"`、
`"knife-drop-rules"`、`"compat-util"`、`"recipes"`、`"item-displays"`、`"scheduler"`、`"buffs"`、
`"debug-tools"`）；未知或 null 的 id 返回 `false`。

在早于引入这两个方法的构建上，调用会抛 `NoSuchMethodError` —— 捕获它并当作修订 0。`FDAddonTemplate.onEnable()`
里就是这个模式：
...
- `api.recipe.*` —— `RecipeType`、`ViewableRecipe`、`RecipeFiller`、`RecipeEditor`、`EditableRecipe`、`NumericField`、`JumpTarget`、`IngredientMatching`、`RecipeBookLayout`（给你的类型一套独立配方书布局，见 `ExampleRecipeType`），以及只读的 `FarmersDelightRecipes`（查询 FD 自身的炖锅 / 切菜板配方）。
  ...
  - **填充反馈**：`RecipeFiller` 可重写 `fillDetailed(player, recipe, fillAll)` 返回一个 `FillOutcome`（`FILLED` / `MISSING_INGREDIENTS` / `INVENTORY_FULL` / `NOTHING`），使配方书镜像炖锅填充按钮的各种状态；`fillAll` 区分 shift 点击（整份配方）与单击（每种各一个）。默认实现会适配布尔版 `fill(...)`，因此老式 filler 仍可用——只是没有更丰富的状态。只有 `FILLED` 应重新打开工作站 GUI，其余留在配方书上让反馈可见。
...
- `api.util.*` —— `PluginManagerGuard`（阻止 PlugMan 之类在运行时重载 / 卸载你的插件；在 onEnable 注册）、`TooltipUtils.hideDurabilityLine`（当物品的耐久值被用作某种计量条时隐藏耐久行——见 `util/ExampleTooltipCustomizer`），以及跨版本兼容垫片 `CompatAttributes`（跨 1.21.2 属性注册表重命名解析 `MAX_HEALTH` / `ATTACK_SPEED`——传给 `getAttribute` 前先判空）与 `CompatItemMeta`（`setItemModel` / `isSupported`，在 1.21.4+ 上施加 `item_model` 组件、在 1.21 / 1.21.1 上为空操作）。请用这些共享垫片，而不是自己另留一份。
...
模板已将其作为 [`util/ExampleConfigBootstrap`](src/main/java/com/example/fdaddon/util/ExampleConfigBootstrap.java)
发布，以 BrewinAndChewin 生产环境的 `BrewinConfigBootstrap` 为范本；`FDAddonTemplate.onEnable()` 会在
`saveDefaultConfig()` 之后、读取任何值之前调用 `configBootstrap.updateConfig()`。其形态：
...
### 方块工作站查询 —— `api.block.FarmersDelightBlocks`（+ `FarmersDelightStation`、`*Snapshot`）

只读地识别 FarmersDelight 的四个工作站并读取它们当前所持内容。所有参数与返回类型都是 Bukkit / java 类型
（外加几个快照 record），因此**无需** CraftEngine 依赖。见 `FDAddonTemplate.java` 里的 `readCookingPot`。
...
if (FarmersDelightBlocks.isStation(block)) {
    ...
    String blockId = FarmersDelightBlocks.blockIdOf(block);              // 任意 CE 自定义方块 id，或 null
    CookingPotSnapshot pot = FarmersDelightBlocks.cookingPot(block);     // 除非它是炖锅，否则为 null
    if (pot != null && pot.cooking()) {
        double p = pot.progressFraction();                               // 0..1
    }
}
// 还有：cuttingBoard(block) -> CuttingBoardSnapshot、skillet(block) -> SkilletSnapshot、stove(block) -> StoveSnapshot、
// stationIdOf(block)。
```

来自 API 自身 javadoc 的两条硬规则：

- **绝不要用 `Material` 识别工作站。** CraftEngine 会报告一个可配置的伪装材质（默认对所有自定义方块都是同一个），
  因此 `getType() == Material.X` 什么都识别不出，且当服主改动 CraftEngine 的伪装设置时会静默变化。始终走
  `isStation` / `stationOf` / `stationIdOf`，它们解析真实的自定义方块状态（识别依据是附加的 CraftEngine
  behavior，因此换皮或附属提供的工作站仍能识别）。`FarmersDelightStation.defaultBlockId()` 是 FD 发布每个工作站
  时所用的原版 id —— 对换皮方块而言并不是 `stationIdOf` 返回的那个。
- **在拥有该方块的区域线程上调用**（Folia / Luminol）：每个方法都读世界状态，所以要在该方块处事件的监听器里、
  或用 `runAtLocation` 调度的任务里调用。从别的区域、全局区域或异步任务调用会撞上 Paper 的线程检查并抛异常。
  快照是调用瞬间拍下的一份拷贝（record 不可变、不回写）—— 把它当作一次读数，而非活动句柄。`null` 表示
  “不是该工作站，或方块实体未加载”。

### 刀掉落 —— `api.loot.FarmersDelightKnifeDrops`

在运行时注册“用刀采收”的额外生物掉落（FD 火腿 / 皮革 / 羽毛 / 线掉落背后的那套规则），适合自带屠宰物品的
附属，或添加条件掉落的任务插件。规则在“玩家用匹配工具击杀匹配类型的成年实体且概率判定通过”时触发；每个实体
类型只有一条规则。这里注册的规则可在 `/fd reload` 后保留（重载会在 `config.yml` 之上重新施加它们），因此请在
插件禁用时反注册。

```java
FarmersDelightKnifeDrops.register("pig", "myaddon:raw_ham", "myaddon:cooked_ham", 0.35, 0.05);
// 完整形式可加逐规则的工具匹配器（物品 id + "#tag" id）；空/null 则回退到全局刀具列表：
FarmersDelightKnifeDrops.register("cow", "myaddon:tallow", null, 0.20, 0.0,
        List.of("myaddon:cleaver"), List.of("minecraft:swords"));
FarmersDelightKnifeDrops.unregister("pig");                 // 只移除本 facade 注册的规则
boolean has = FarmersDelightKnifeDrops.hasRule("cow");      // 来自任意来源（config / 内建 / facade）
```
